...
import sys
import traceback
from os.path import abspath, basename, dirname, exists, getmtime, isdir, join, split
...
if len(sys.argv) >= 2 and (
    "--help" in sys.argv[1:]
    or sys.argv[1] in ("--help-commands", "egg_info", "--version", "clean", "sdist")
):
    MODE = "info"
...
class GenDocs(Command):
    ...
    def mlab_reference(self):
        ...
        if (
            self.latest_modified(
                source_path, filetypes=sources, ignore_dirs=excluded_dirs
            )[0]
            > target_time
    ...
    def example_files(self):
        ...
        if (
            ...
            or not exists(join("docs", "source", "mayavi", "auto", "examples.rst"))
        ):
            try:
                from mayavi import mlab
                from mayavi.tools import auto_doc
...
class MyClean(clean.clean):
    ...
    def run(self):
        ...
        ext_file = os.path.join(
            MY_DIR, "tvtk", "array_ext" + (".pyd" if sys.platform == "win32" else ".so")
        )
...
if HAS_NUMPY:
    ...
    config = {"packages": []}
packages = setuptools.find_packages(exclude=config["packages"] + ["docs", "examples"])
