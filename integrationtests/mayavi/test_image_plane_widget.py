...
class TestImagePlaneWidget(TestCase):
    def make_data(self):
        ...
        x = numpy.reshape(numpy.arange(-5.0, 5.0 + dx * 0.5, dx, "f"), (dims[0], 1, 1))
        y = numpy.reshape(numpy.arange(-5.0, 5.0 + dy * 0.5, dy, "f"), (1, dims[1], 1))
        z = numpy.reshape(numpy.arange(-5.0, 5.0 + dz * 0.5, dz, "f"), (1, 1, dims[0]))
