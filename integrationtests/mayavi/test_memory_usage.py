...
class TestMemoryUsage(TestCase):
    ...
    def find_memory_usage(self):
        ...
        return float(process.memory_info()[0]) / (1024 ** 2)

    def test_memory_usage(self):
        warning =  """ 
                   =========================================================
                   WARNING: This is a test for memory usage and memory leak
                   detection. AS memory leaks still exist, the assertions in
                   this test use > operator. When the memory leaks are fixed
                   completely, the assertion should be == with a minimal 
                   percent variance.'
                   =========================================================
                   """
        print warning
        mem_usage_before_sample_runs = self.find_memory_usage()
        for i in range(1, 11):
            before_mem_usage = self.find_memory_usage()
            self.main()
            after_mem_usage = self.find_memory_usage()
            assert after_mem_usage > before_mem_usage
            leak = after_mem_usage - before_mem_usage
            print 'Memory leak for sample run %s is %5.1fMB' % (i, leak)
        mem_usage_after_sample_runs = self.find_memory_usage()
        leak = mem_usage_after_sample_runs - mem_usage_before_sample_runs
        assert mem_usage_after_sample_runs >  mem_usage_before_sample_runs
        print '+++ Memory leak for sample runs is %5.1fMB' % leak
