...
gap = 2.0 * d  # distance between two parts of upper, U-shaped assembly
...
L2display = L2 + d / 2.0  # show lower bar a bit longer than physical, to overlap axle
# Coefficients used in Lagrangian calculation
A = (1.0 / 4.0) * M1 * L1 ** 2 + (1.0 / 12.0) * M1 * L1 ** 2 + M2 * L1 ** 2
B = (1.0 / 2.0) * M2 * L1 * L2
C = g * L1 * (M1 / 2.0 + M2)
D = M2 * L1 * L2 / 2.0
E = (1.0 / 12.0) * M2 * L2 ** 2 + (1.0 / 4.0) * M2 * L2 ** 2
F = g * L2 * M2 / 2.0

hpedestal = 1.3 * (L1 + L2)  # height of pedestal
wpedestal = 0.1  # width of pedestal
tbase = 0.05  # thickness of base
wbase = 8.0 * gap  # width of base
offset = 2.0 * gap  # from center of pedestal to center of U-shaped upper assembly
top = vector(0, 0, 0)  # top of inner bar of U-shaped upper assembly

theta1 = 1.3 * pi / 2.0  # initial upper angle (from vertical)
...
pedestal = box(
    pos=(top - vector(0, hpedestal / 2.0, offset)),
    size=(wpedestal, 1.1 * hpedestal, wpedestal),
    color=(0.4, 0.4, 0.5),
)

base = box(
    pos=(top - vector(0, hpedestal + tbase / 2.0, offset)),
    size=(wbase, tbase, wbase),
    color=(0.4, 0.4, 0.5),
)

bar1 = box(
    pos=(L1display / 2.0 - d / 2.0, 0, -(gap + d) / 2.0),
    size=(L1display, d, d),
    color=(1, 0, 0),
)
bar1b = box(
...
bar2 = box(
    pos=(L2display / 2.0 - d / 2.0, 0, 0), size=(L2display, d, d), color=(0, 1, 0)
)
