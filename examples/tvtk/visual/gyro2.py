...
from tvtk.tools.visual import vector, box, cylinder, frame, curve, iterate, show
...
wbase = 3.0 * wpedestal  # width of base
g = 9.8
Fgrav = vector(0, -M * g, 0)
top = vector(0, 0, 0)  # top of pedestal

theta = pi / 3.0  # initial polar angle of shaft (from vertical)
...
phi = -pi / 2.0  # initial azimuthal angle
...
pedestal = box(
    pos=top - vector(0, hpedestal / 2.0, 0),
    height=hpedestal,
    length=wpedestal,
    width=wpedestal,
    color=(0.4, 0.4, 0.5),
...
base = box(
    pos=top - vector(0, hpedestal + tbase / 2.0, 0),
    height=tbase,
    length=wbase,
    width=wbase,
...
shaft = cylinder(axis=(Lshaft, 0, 0), length=Lshaft, radius=Rshaft, color=(0, 1, 0))
rotor = cylinder(
...
trail = curve(radius=Rshaft / 8.0, color=(1, 1, 0))
...
def anim():
    ...
    for step in range(Nsteps):  # multiple calculation steps for accuracy
        ...
        atheta = (
            phidot ** 2 * sin(theta) * cos(theta)
            - 2.0 * (alphadot + phidot * cos(theta)) * phidot * sin(theta)
            + 2.0 * M * g * r * sin(theta) / I
        )
        aphi = 2.0 * thetadot * (alphadot - phidot * cos(theta)) / sin(theta)
