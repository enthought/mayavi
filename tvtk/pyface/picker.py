...
class DefaultPickHandler(PickHandler):
    ...
    coordinate = Trait(
        None, None, Array("d", (3,)), desc="the coordinate of the picked point"
    )

    scalar = Trait(None, None, Array, Float, desc="the scalar at picked point")

    vector = Trait(None, None, Array("d", (3,)), desc="the vector at picked point")

    tensor = Trait(None, None, Array("d", (3, 3)), desc="the tensor at picked point")

    # History of picked data.
    history = Str

    default_view = View(
        Item(name="ID", style="readonly"),
...
class Picker(HasTraits):
    ...
    pick_type = Trait(
        "point",
        TraitRevPrefixMap({"point_picker": 1, "cell_picker": 2, "world_picker": 3}),
    ...
    default_view = View(
        Group(
            Group(Item(name="pick_type"), Item(name="tolerance"), show_border=True),
            Group(
                Item(name="pick_handler", style="custom"),
                show_border=True,
                show_labels=False,
            ),
            Group(Item(name="show_gui"), Item(name="auto_raise"), show_border=True),
        ),
        resizable=True,
        buttons=["OK"],
