...
class TestVTKParser(unittest.TestCase):
    ...
    def test_parse(self):
        ...
        self.assertEqual(
            p.get_toggle_methods(), {"Debug": 0, "GlobalWarningDisplay": 1}
        )
        ...
        res = {"EdgeVisibility": 0, "BackfaceCulling": 0, "FrontfaceCulling": 0}
        ...
        if hasattr(obj, "GetTexture"):
            ...
            if hasattr(obj, "GetMaterialName"):
                ...
                self.assertTrue(
                    all([x in expect for x in sorted(p.get_get_methods())]), msg
                )
            else:
    ...
    def test_method_signature(self):
        ...
        if len(sig) == 1:
            ...
            self.assertEqual(
                [([None], ["vtkDataObject"]), ([None], ("int", "vtkDataObject")),], sig
            )

        self.assertEqual(
            [(["vtkPolyData"], None), (["vtkPolyData"], ["int"])],
            p.get_method_signature(o.GetOutput),
        )

        # Test if function arguments work.
        self.assertEqual(
            [(["int"], ("int", "function"))], p.get_method_signature(o.AddObserver)
        )
