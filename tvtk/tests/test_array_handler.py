...
class TestArrayHandler(unittest.TestCase):
    ...
    def test_array2vtk(self):
        ...
        t_z.append(numpy.array([[1.0, 2.0, 3.0, 400.0], [4, 5, 6, 700]], "d"))

        # Test if arrays with number of components not in [1,2,3,4,9] work.
        t_z.append(numpy.array([[1, 2, 3, 400, 5000], [4, 5, 6, 700, 8000]], "d"))
        ...
        for z in t_z:
            ...
            self.assertEqual(
                vtk_arr.GetReferenceCount(), array_handler.BASE_REFERENCE_COUNT
            )
        ...
        a[0] = [10.0, 20.0, 30.0]
        self.assertEqual(vtk_arr.GetTuple3(0), (10.0, 20.0, 30.0))
        ...
        float_types = [
            x
            for x in numpy.sctypes["float"]
            if x().dtype.name not in ("float16", "float128")
        ]
        for dtype in numpy.sctypes["int"] + numpy.sctypes["uint"] + float_types:
            print(dtype)
    ...
    def test_arr2vtkPoints(self):
        ...
        self.assertRaises(
            AssertionError, array_handler.array2vtkPoints, [0.0, 1.0, 1.0]
        )
    ...
    def test_arr2vtkIdList(self):
        ...
        self.assertRaises(AssertionError, array_handler.array2vtkIdList, [[1, 2, 3]])
    ...
    def test_get_correct_sig(self):
        ...
        for i in range(len(sigs)):
            if res[i] is TypeError:
                self.assertRaises(
                    res[i], array_handler.get_correct_sig, args[i], sigs[i]
                )
            else:
    ...
    def test_deref_array(self):
        ...
        self.assertEqual(
            mysum(array_handler.vtk2array(r[0].GetData()) - numpy.array(args[3], "f")),
            0,
        )
