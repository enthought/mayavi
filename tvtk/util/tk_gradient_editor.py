...
class GradientControl(tk.Frame):
    ...
    def __init__(self, master, gradient_table, width, height):
        ...
        self.canvas = tk.Canvas(self, background="white", width=width, height=height)
        self.canvas.pack()
        self.update()
...
class FunctionControl(tk.Frame):
    ...
    class Channel:
        def __init__(
            self, function_control, name, color_string, channel_index, channel_mode
        ):
        ...
        def paint(self, canvas):
            ...
            for control_point in relevant_control_points:
                ...
                canvas.create_rectangle(
                    x - radius, y - radius, x + radius, y + radius, outline="#000000"
                )

    def __init__(
        self, master, gradient_table, color_space, width, height, on_table_changed=None
    ):
        tk.Frame.__init__(self, master, borderwidth=2, relief="groove")
        ...
        self.canvas = tk.Canvas(
            self, background="white", width=self.width, height=self.height
        )
        self.canvas.pack()
        self.channels = []
    ...
    def find_control_point(self, x, y):
        ...
        for channel in self.channels:
            for control_point in self.table.control_points:
                ...
                if (point_x - x) ** 2 + (
                    point_y - y_
                ) ** 2 <= self.control_pt_click_tolerance ** 2:
...
class GradientEditor(tk.Toplevel):
    ...
    def __init__(self, master, vtk_table, on_change_color_table=None):
        ...
        self.minsize(
            gradient_preview_width + 4,
            gradient_preview_height + 5 * channel_function_height + 50,
        )
        ...
        # create controls.
        self.gradient_control = GradientControl(
        ...
        instruction_label = tk.Label(
            button_frame, text="left button: move point; right click: toggle point"
        )
        ...
        function_edit = tk.Entry(
            nonlin_frame, width=35, textvariable=self.nonlinear_function_string
        )
        ...
        label = tk.Label(
            nonlin_frame,
            text="f(x) should map [0..1] to [0..1]. It rescales the gradient.",
        )
