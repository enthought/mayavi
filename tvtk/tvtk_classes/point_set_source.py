...
class PointSetSource(Source):
    """
    PointSetSource - abstract class whose subclasses generate point
    data
    
    Superclass: Source
    
    PointSetSource is an abstract class whose subclasses generate
    pointdata.
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkPointSetSource, obj, update, **traits)
    ...
    def set_output(self, obj):
        """
        V.set_output(PointSet)
        C++: void SetOutput(PointSet *output)
        Get the output of this source.
        """
        old_val = self._get_output()
        self._wrap_call(self._vtk_obj.SetOutput, deref_vtk(obj))
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(PointSetSource, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            traitsui.View((traitsui.Item("handler._full_traits_list",show_label=False)),
            title='Edit PointSetSource properties', scrollable=True, resizable=True,
