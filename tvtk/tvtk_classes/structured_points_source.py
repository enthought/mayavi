...
class StructuredPointsSource(Source):
    """
    StructuredPointsSource - Abstract class whose subclasses generates
    structured Points data
    
    Superclass: Source
    
    StructuredPointsSource is an abstract class whose subclasses
    generate structured Points data.
    
    See Also:
    
    StructuredPointsReader PLOT3DReader
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkStructuredPointsSource, obj, update, **traits)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(StructuredPointsSource, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            traitsui.View((traitsui.Item("handler._full_traits_list",show_label=False)),
            title='Edit StructuredPointsSource properties', scrollable=True, resizable=True,
