...
class GenericDataSetAlgorithm(Algorithm):
    """
    GenericDataSetAlgorithm - Objects that generate adapted data sets
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkGenericDataSetAlgorithm, obj, update, **traits)
    ...
    def get_output(self, idx=None):
        """
        V.get_output() -> GenericDataSet
        C++: GenericDataSet *GetOutput()
        V.get_output(int) -> GenericDataSet
        C++: GenericDataSet *GetOutput(int)
    ...
    def get_generic_data_set_input(self, *args):
        """
        V.get_generic_data_set_input(int) -> GenericDataSet
        C++: GenericDataSet *GetGenericDataSetInput(int port)"""
        ret = self._wrap_call(self._vtk_obj.GetGenericDataSetInput, *args)
        return wrap_vtk(ret)
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(GenericDataSetAlgorithm, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            traitsui.View((traitsui.Item("handler._full_traits_list",show_label=False)),
            title='Edit GenericDataSetAlgorithm properties', scrollable=True, resizable=True,
