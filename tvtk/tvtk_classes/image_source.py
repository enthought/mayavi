...
class ImageSource(Source):
    """
    ImageSource - Source of data for the imaging pipeline
    
    Superclass: Source
    
    ImageSource is the superclass for all imaging sources and filters.
    ...
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkImageSource, obj, update, **traits)
    ...
    def set_output(self, obj):
        """
        V.set_output(ImageData)
        C++: void SetOutput(ImageData *output)
        Get the output of this source.
        """
        old_val = self._get_output()
        self._wrap_call(self._vtk_obj.SetOutput, deref_vtk(obj))
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(ImageSource, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            traitsui.View((traitsui.Item("handler._full_traits_list",show_label=False)),
            title='Edit ImageSource properties', scrollable=True, resizable=True,
