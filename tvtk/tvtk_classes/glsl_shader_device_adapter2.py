...
class GLSLShaderDeviceAdapter2(Object):
    """
    GLSLShaderDeviceAdapter2 - adapter to pass generic vertex 
    
    Superclass: Object
    
    ShaderDeviceAdapter subclass for ShaderProgram2.
    
    """
    def __init__(self, obj=None, update=True, **traits):
        tvtk_base.TVTKBase.__init__(self, vtk.vtkGLSLShaderDeviceAdapter2, obj, update, **traits)
    ...
    def send_attribute(self, *args):
        ...
        open_gl ID for the attribute will simply be cached.
        """
    ...
    def trait_view(self, name=None, view_element=None):
        if view_element is not None or name not in (None, '', 'traits_view', 'full_traits_view', 'view'):
            return super(GLSLShaderDeviceAdapter2, self).trait_view(name, view_element)
        if name == 'full_traits_view':
            full_traits_view = \
            traitsui.View((traitsui.Item("handler._full_traits_list",show_label=False)),
            title='Edit GLSLShaderDeviceAdapter2 properties', scrollable=True, resizable=True,
