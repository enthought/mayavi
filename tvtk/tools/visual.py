...
class Animator(HasTraits):
    ...
    event_group = Group(
        Item("start_animation", style="simple"),
        Item("_"),
...
class Curve(HasTraits):
    ...
    def __init__(self, **traits):
        ...
        if axis is not None:
            self._axis_changed(
                numpy.array((1.0, 0.0, 0.0)), numpy.asarray(axis, dtype=float)
            )
    ...
    def rotate(self, angle, axis, origin=numpy.array([0.0, 0.0, 0.0])):
        ...
        p, pi, ax = rotate(
            axis, angle, origin, self.pos, self.points, numpy.array([0.0, 0.0, 0.0])
        )
...
class Cone(HasTraits):
    ...
    def _radius_changed(self):
        points, lines = self._create_points(
            self.radius, self.height, self.pos, self.axis
        )
...
class Ellipsoid(HasTraits):
    ...
    def _size_changed(self, value):
        points, polys = self._create_points(self.radius, self.pos)
        self.trait_set(
            length=value[0], height=value[1], width=value[2], trait_change_notify=False
        )
        self.polydata.points = points
        self.polydata.modified()
        self.normals.update()
