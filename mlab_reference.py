...
OUT_DIR = os.sep.join(
    [os.path.dirname(os.path.abspath(__file__)), DEFAULT_INPUT_DIR, "mayavi", "auto"]
)
...
def is_valid_rst(string):
    ...
    compiled_rst = publisher.reader.read(
...
%s
        ...
        image_file = (
            self.image_dir
            + os.sep
            + self.module.__name__.replace(".", "_")
            + "_"
            + func_name
            + ".jpg"
        )
        ...
        documentation = document_function(
            func, func_name=func_name, example_code=example_code, image_file=image_file
        )
...
"""
    ...
    def write_doc_submodule(
        self, filename, title=None, header=None, submodulename=None
    ):
...
"""
        ...
        for func_name in sorted(self.to_document):
            ...
            if submodulename is not None and not func.__module__ == submodulename:
                continue
...
if __name__ == "__main__":
    ...
    from mayavi.tools import helper_functions, camera, decorations, figure
...
""",
        ...
        sub_filenames=[
            "mlab_%s.rst" % module.__name__.split(".")[-1] for module in sub_modules
        ],
        sub_modules_titles=[
            "Plotting functions",
    ...
    from mayavi.tools import pipeline, sources, tools, probe_data
