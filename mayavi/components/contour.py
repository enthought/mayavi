...
from traits.api import Instance, List, Tuple, Bool, Range, Float, Property
...
from mayavi.components.common import get_module_source, convert_to_poly_data
...
class Contour(Component):
    ...
    filled_contours = Bool(False, desc="if filled contours are to be generated")

    # Specify if contours are generated explicitly or automatically.
    auto_contours = Bool(
        False, desc="if contours are given explicitly or automatically computed"
    )

    # Number of contours, used when `auto_contours` are chosen.
    number_of_contours = Range(
        1, 100000, enter_set=True, auto_set=False, desc="number of contours to generate"
    )
    ...
    auto_update_range = Bool(True, desc="if the contour range is updated automatically")
    ...
    def _contours_items_changed(self, list_event):
        ...
        added, removed, index = (list_event.added, list_event.removed, list_event.index)
    ...
    def _update_ranges(self):
        ...
        if rng != self._current_range:
            self.trait_set(
                _data_min=rng[0], _data_max=rng[1], trait_change_notify=False
            )
    ...
    def _do_auto_contours(self):
        ...
        if self.auto_contours:
            ...
            self.contour_filter.generate_values(
                self.number_of_contours, min(minc, maxc), max(minc, maxc)
            )
            self.data_changed = True
    ...
    def _clip_contours(self, rng):
        ...
        if self.auto_contours or ctr != self.contours:
            self.contours = ctr
            self.trait_set(
                minimum_contour=self._data_min,
