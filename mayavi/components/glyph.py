...
class Glyph(Component):
    ...
    color_mode = Trait(
        "color_by_scalar",
        TraitRevPrefixMap(
            {"color_by_vector": 2, "color_by_scalar": 1, "no_coloring": 0}
        ),
        desc="if coloring is done by scalar or vector/normal magnitude",
    )
    color_mode_tensor = Trait(
        "scalar",
        TraitRevPrefixMap({"scalars": 1, "eigenvalues": 2, "no_coloring": 0}),
    ...
    view = View(
        ...
        Group(
            ...
            Group(
                Item(name="glyph", style="custom", resizable=True), show_labels=False
            ),
            label="Glyph",
            selected=True,
        ),
