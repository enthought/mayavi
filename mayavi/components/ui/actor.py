...
from traitsui.api import View, Group, Item, InstanceEditor, DropEditor, Tabbed
...
if VTK_VER[:3] in ["4.2", "4.4"]:
    ...
    _mapper_base_group = Group(
        Item(name="scalar_visibility"), Item(name="interpolate_scalars_before_mapping"),
    )
...
_mapper_group = Group(
    ...
    Item(
        name="mapper", show_label=False, editor=InstanceEditor(label="More options ...")
    ),
    show_border=True,
    label="Mapper",
)
...
_actor_group = Group(
    ...
    Item(
        name="actor", show_label=False, editor=InstanceEditor(label="More options ...")
    ),
    show_border=True,
    label="Actor",
)
...
actor_group = Group(
    _actor_group, _mapper_group, _prop_group, label="Actor", show_labels=False,
)

texture_group = Group(
    Item(name="enable_texture"),
    Group(
        Item(name="texture_source_object", editor=DropEditor()),
