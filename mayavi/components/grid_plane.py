...
class GridPlane(Component):
    ...
    axis = Enum("x", "y", "z", desc="specifies the axis normal to the grid plane")

    # The position of the grid plane.
    position = Range(value=0, low="_low", high="_high", enter_set=True, auto_set=False)
    ...
    view = View(
        Group(Item(name="axis"), Item(name="position", enabled_when="_high > 0"))
    )
