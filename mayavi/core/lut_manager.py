...
try:
    ...
    message = (
        "Failed to load pylab colormaps from file:\n"
        "{filepath}\n"
...
def check_lut_first_line(line, file_name=""):
    ...
    if first[0] != "LOOKUP_TABLE":
        ...
        errmsg = (
            errmsg + "is not a proper lookup table file."
...
def lut_mode_list():
    ...
    """
    lut_mode_list = ["blue-red", "black-white", "file",] + list(pylab_luts.keys())
    lut_mode_list.sort()
...
class LUTManager(Base):
    ...
    scalar_bar_representation = Instance(tvtk.Object, allow_none=True, record=True)
    ...
    number_of_labels = Range(
        0, 64, 8, enter_set=True, auto_set=False, desc="the number of labels to display"
    )
    ...
    default_data_name = Str(
        "data", enter_set=True, auto_set=False, desc="the default data name"
    )
    ...
    create_lut = Button(
        "Launch LUT editor",
        desc="if we launch a Lookup table editor in" " a separate process",
    )
    ...
    def __init__(self, **traits):
        ...
        pc.trait_set(coordinate_system="normalized_viewport", value=(0.1, 0.01, 0.0))
    ...
    def _create_lut_fired(self):
        ...
        script = os.path.join(
            os.path.dirname(util.__file__), tk + "_gradient_editor.py"
        )
