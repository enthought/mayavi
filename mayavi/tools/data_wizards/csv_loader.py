...
from mayavi.tools.data_wizards.csv_sniff import Sniff, loadtxt, array2dict
...
class CSVLoader(HasTraits):
    ...
    skiprows = Int(desc="The number of rows to skip at the beginning of the file")
    ...
    def guess_defaults(self):
        try:
            ...
            kwds = {"comments": "#", "delimiter": ",", "dtype": float, "skiprows": 0}
        ...
        self.columns = [
            ListItem(
                name="Column %i:" % (i + 1), parent=self, column_number=i, my_name=val
            )
    ...
    def default_traits_view(self):
        view = View(
            VSplit(
                ...
                Group(
                    Group(
                        Item("data", show_label=False, editor=self.tabular_editor,),
                        label="Preview table",
                    ),
                    Group(
                        Item(
