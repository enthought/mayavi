...
class DataSourceWizard(HasTraits):
    ...
    def _get_grid_shape_source(self):
        if self.grid_shape_source_ == "":
            ...
            if not self.grid_shape.any():
                self.grid_shape = self.data_sources[keys[0]].shape
            return keys[0]
    ...
    def _grid_shape_source_changed(self):
        if not self.grid_shape_source == "":
            array_shape = atleast_3d(self.data_sources[self.grid_shape_source]).shape
            grid_shape = ones((3,))
    ...
    def _get__grid_shape_source_labels(self):
        values = ['Shape of array: "%s"' % name for name in self._data_sources_names]
        values.sort
    ...
    scalar_data = Str(
        "", help="Select the array that gives the value of the " "scalars plotted."
    )

    position_x = Str(
        help="Select the array that gives the x " "position of the data points"
    )

    position_y = Str(
        help="Select the array that gives the y " "position of the data points"
    )

    position_z = Str(
        help="Select the array that gives the z " "position of the data points"
    )
    ...
    def build_data_source(self):
        ...
        if self.position_type_ == "image data" and not self.data_type_ == "point":
        ...
        if self.position_type_ == "explicit" and self.data_type_ == "surface":
            factory.connectivity_triangles = self.get_data(self.connectivity_triangles)
    ...
    def check_arrays(self):
        ...
        for attr in arrays_to_check:
            if not self.get_data(getattr(self, attr)).size == size:
                return False
        if self.data_type_ == "surface" and self.position_type_ == "explicit":
...
class DataSourceWizardView(DataSourceWizard):
    ...
    def _get__data_sources_wrappers(self):
        return [
            ArrayColumnWrapper(name=name, shape=repr(self.data_sources[name].shape))
    ...
    _info_image = Instance(ImageResource, ImageLibrary.image_resource("@std:alert16",))

    # ----------------------------------------------------------------------
    # TraitsUI views
    # ----------------------------------------------------------------------
    _coordinates_group = HGroup(
        Item(
            "position_x",
            label="x",
            editor=EnumEditor(name="_data_sources_names", invalid="_is_not_ok"),
        ),
        Item(
            "position_y",
            label="y",
        ...
        Group(
            ...
            Group(
                Item(
                    ...
                    label="Grid shape",
                    editor=EnumEditor(
                        name="_grid_shape_source_labels", invalid="_is_not_ok"
                    ),
                ),
                HGroup(
                    spring,
    ...
    _connectivity_group = (
        Group(
            HGroup(
                Item("_connectivity_text", style="readonly", resizable=False),
                spring,
                Item(
                    "connectivity_triangles",
    ...
    _scalar_data_group = Group(
        Item("_scalar_data_text", style="readonly", resizable=False, show_label=False),
        HGroup(
            spring,
            Item(
                "scalar_data",
    ...
    _optional_scalar_data_group = (
        Group(
            HGroup(
                "has_scalar_data",
                Item("_optional_scalar_data_text", resizable=False, style="readonly"),
                show_labels=False,
            ),
            Item(
                "_scalar_data_text",
    ...
    _array_view = View(
        Item("_array_label", editor=TitleEditor(), show_label=False),
        Group(
            Item(
                "_data_sources_wrappers",
                editor=TabularEditor(adapter=ArrayColumnAdapter(),),
            ),
            show_border=True,
            show_labels=False,
        ),
    ...
    _questions_view = View(
        ...
        HGroup(
            ...
            Group(
                ...
                Item("_preview_window", style="custom", label="Preview structure"),
                show_labels=False,
    ...
    _point_data_view = View(
        Group(
            ...
            HGroup(
                "lines",
                Item("_lines_text", style="readonly", resizable=False),
                label="Lines",
                show_labels=False,
                show_border=True,
            ),
    ...
    _vector_data_view = View(
        Group(_vector_data_group, _position_group, _optional_scalar_data_group,)
    )

    _volumetric_data_view = View(
        Group(_scalar_data_group, _position_group, _optional_vector_data_group,)
    )
    ...
    _wizard_view = View(
        Group(
            ...
            HGroup(
                Item("_info_image", editor=ImageEditor(), visible_when="_is_not_ok"),
                Item(
                    "_info_text",
                    style="readonly",
                    resizable=False,
    ...
    def preview(self):
        ...
        if not (self.has_vector_data or self.data_type_ == "vector"):
            ...
            g.actor.property.point_size = 3.0
