...
class ModuleFactory(PipeFactory):
    ...
    color = Trait(
        None,
        None,
        TraitTuple(Range(0.0, 1.0), Range(0.0, 1.0), Range(0.0, 1.0)),
    ...
    opacity = CFloat(1.0, desc="""The overall opacity of the vtk object.""")
    ...
    line_width = CFloat(2.0, desc=""" The width of the lines, if any used.""")
...
class DataModuleFactory(ModuleFactory):
    ...
    def _transparent_changed(self):
        if self.transparent:
            data_range = self._target.module_manager.scalar_lut_manager.data_range
            self._target.module_manager.scalar_lut_manager.lut.alpha_range = (0.2, 0.8)
            data_range = numpy.mean(data_range) + 0.4 * (
                data_range.max() - data_range.min()
            ) * numpy.array([-1, 1])
            self._target.module_manager.scalar_lut_manager.data_range = data_range

    colormap = Trait("blue-red", lut_mode_list(), help="""type of colormap to use.""")
    ...
    def _vmin_changed(self):
        if self.vmin is None and self.vmax is None:
            self._target.module_manager.scalar_lut_manager.use_default_range = True
            return

        self._target.module_manager.scalar_lut_manager.use_default_range = False
        vmin, vmax = self._target.module_manager.scalar_lut_manager.data_range
        ...
        if self.vmax is not None:
            ...
            if self.vmin is None:
                vmin = min(vmin, vmax)
        self._target.module_manager.scalar_lut_manager.data_range = (vmin, vmax)
...
class ContourModuleFactory(DataModuleFactory):
    ...
    def _contours_changed(self):
        ...
        if contour_list:
            ...
            assert (
                type(self.contours) == int
            ), "The contours argument must be an integer"
            assert self.contours > 0, "The contours argument must be positive"
            self._target.contour.trait_set(
                auto_contours=True, number_of_contours=self.contours
            )
...
class CutPlaneFactory(DataModuleFactory):
    ...
    view_controls = Bool(
        ...
        desc=("Whether or not the controls of the " "cut plane are shown."),
    )

    def _plane_orientation_changed(self):
        choices = dict(
            x_axes=numpy.array([1.0, 0.0, 0.0]),
            y_axes=numpy.array([0.0, 1.0, 0.0]),
            z_axes=numpy.array([0.0, 0.0, 1.0]),
        )
        self._target.implicit_plane.normal = choices[self.plane_orientation]
...
class VectorsFactory(DataModuleFactory):
    ...
    mode = Trait("2darrow", glyph_mode_dict, desc="""the mode of the glyphs.""")
...
class StreamlineFactory(DataModuleFactory):
    ...
    seed_visible = Bool(
        True, adapts="seed.widget.enabled", desc="Control the visibility of the seed.",
    )

    seed_scale = CFloat(1.0, desc="Scales the seed around its default center",)

    seed_resolution = Either(
        None,
        CInt,
        desc="The resolution of the seed. Determines the number of " "seed points",
    )
    ...
    def _seedtype_changed(self):
        ...
        # being called anyhow.
        self._target.seed.widget = widget = self._target.seed.widget_list[
            self.seedtype_
        ]
        scene = self._target.scene

        if not self.seed_scale == 1.0:
        ...
        if self.seed_resolution is not None:
            ...
            if self.seedtype in ("plane", "line"):
                ...
                widget.phi_resolution = widget.theta_resolution = self.seed_resolution
...
class ImageActorFactory(DataModuleFactory):
    ...
    opacity = Range(
        0.0, 1.0, 1.0, adapts="actor.opacity", desc="""the opacity of the image."""
    )
...
class VolumeFactory(PipeFactory):
    ...
    def _vmin_changed(self):
        ...
        if self.color is None and ((self.vmin is not None) or (self.vmax is not None)):
            ...
            if hasattr(self._target._ctf, "nodes"):
                ...
                for value in self._target._ctf.nodes:
                    r, g, b = self._target._ctf.get_color(value)
