...
from traits.api import HasPrivateTraits, Str, TraitError, Instance, Any, Bool
...
class PipeFactory(HasPrivateTraits):
    ...
    _engine = Instance(
        Engine, help=("the figure on which the object " "should be added")
    )
    ...
    def add_module(self, parent, kwargs=dict()):
        ...
        if parent is not None:
            ...
            if module_manager is not None and len(module_manager.children) > 0:
                ...
                if "vmin" in kwargs:
                    if (
                        not scalar_lut.use_default_range
                        and kwargs["vmin"] != scalar_lut.data_range[0]
                    ):
                        parent = self._engine.add_module(
                            ModuleManager(), module_manager.parent
                        )
                    elif (
                        not scalar_lut.use_default_range
                        and kwargs["vmin"] != scalar_lut.data_range[0]
                    ...
                    if (
                        not scalar_lut.use_default_range
                        and kwargs["vmax"] != scalar_lut.data_range[1]
                    ):
                        parent = self._engine.add_module(
                            ModuleManager(), module_manager.parent
                        )
    ...
    def __init__(self, parent, **kwargs):
        ...
        if (
            isinstance(parent, (Source, tvtk.DataSet))
        ...
        [traits.pop(key) for key in list(traits.keys()) if key[0] == "_" or key is None]
    ...
    def set(self, trait_change_notify=True, **traits):
        return self.trait_set(trait_change_notify=trait_change_notify, **traits)
