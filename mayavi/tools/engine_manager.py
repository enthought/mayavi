...
def check_backend():
    ...
    if (options.backend != "test" and not options.offscreen) and ETSConfig.toolkit in (
        "null",
        "",
    ):
...
class EngineManager(HasTraits):
    ...
    def get_engine(self):
        ...
        if options.backend == "envisage":
            suitable = [e for e in engines if e.__class__.__name__ == "EnvisageEngine"]
        elif options.backend == "test":
            suitable = [e for e in engines if e.__class__.__name__ == "NullEngine"]
        elif options.offscreen:
            suitable = [e for e in engines if e.__class__.__name__ == "OffScreenEngine"]
    ...
    def show_engine(self, engine=None, rich_view=True):
        ...
        if engine.__class__.__name__ == "EnvisageEngine" or options.backend == "test":
