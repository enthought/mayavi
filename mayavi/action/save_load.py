...
class LoadVisualization(Action):
    ...
    def perform(self, event):
        ...
        parent = self.window.control
        dialog = FileDialog(
            parent=parent, title="Open MayaVi2 file", action="open", wildcard=wildcard
        )
        if dialog.open() == OK:
...
class RunScript(Action):
    ...
    def perform(self, event):
        ...
        parent = self.window.control
        dialog = FileDialog(
            parent=parent, title="Open Python file", action="open", wildcard=wildcard
        )
        if dialog.open() == OK:
            if not isfile(dialog.path):
                error("File '%s' does not exist" % dialog.path, parent)
            ...
            try:
                ...
                exec(compile(open(dialog.path).read(), dialog.path, "exec"), g, g)
