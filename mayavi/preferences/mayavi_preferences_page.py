...
from traits.api import Bool, Enum, Tuple, Range, List, Str, Instance, HasTraits
...
class MayaviRootPreferencesPage(PreferencesPage):
    ...
    # prompted.
    confirm_delete = Bool(
        desc="if the user is prompted before" " a node on the MayaVi tree is deleted"
    )

    # Specifies if the splash screen is shown when mayavi starts.
    show_splash_screen = Bool(desc="if the splash screen is shown at" " startup")

    # Specifies if the adder nodes are shown on the mayavi tree view.
    show_helper_nodes = Bool(
        desc="if the helper (adder) nodes are shown" " on the tree view"
    )
    ...
    use_ipython = Bool(desc="use IPython for the embedded shell " "(if available)")
...
class MayaviMlabPreferencesPage(PreferencesPage):
    ...
    # Offscreen rendering.
    offscreen = Bool(
        desc="if mlab should use offscreen rendering"
