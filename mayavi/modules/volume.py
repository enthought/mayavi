...
from traits.api import Instance, Property, List, ReadOnly, Str, Button, Tuple, Dict
...
def load_volume_prop_from_grad(grad_file_name, volume_prop, scalar_range=(0, 255)):
...
class Volume(Module):
    ...
    volume_mapper_type = DEnum(values_name="_mapper_types", desc="volume mapper to use")
    ...
    lut_manager = Instance(VolumeLUTManager, args=(), allow_none=False, record=True)
    ...
    view = View(
        ...
        Group(
            ...
            Group(
                Item(name="_volume_mapper", style="custom", resizable=True),
                show_labels=False,
            ),
        ...
        Group(
            Item(name="_volume_property", style="custom", resizable=True),
            label="Property",
            show_labels=False,
        ),
        Group(
            Item(
                name="volume", style="custom", editor=InstanceEditor(), resizable=True
            ),
            label="Volume",
            show_labels=False,
        ),
        Group(
            Item(name="lut_manager", style="custom", resizable=True),
            label="Legend",
            show_labels=False,
        ),
        resizable=True,
    ...
    def update_pipeline(self):
        ...
        if dataset.is_a("vtkMultiBlockDataSet"):
            if "MultiBlockVolumeMapper" not in self._available_mapper_types:
                error(
                    "Your version of VTK does not support "
                    "Multiblock volume rendering"
                )
                return
        elif dataset.is_a("vtkUniformGridAMR"):
            if "AMRVolumeMapper" not in self._available_mapper_types:
                error("Your version of VTK does not support " "AMR volume rendering")
                return
        elif ug:
            if not dataset.is_a("vtkImageData") and not dataset.is_a(
                "vtkUnstructuredGrid"
            ):
                error(
                    "Volume rendering only works with "
        ...
        if dataset.is_a("vtkMultiBlockDataSet"):
            ...
            if dataset.point_data.scalars.data_type not in [
                ...
                if "FixedPointVolumeRayCastMapper" in self._available_mapper_types:
                    ...
                    error(
                        "Available volume mappers only work with "
    ...
    def _setup_current_range(self):
        ...
        lm.trait_set(
            default_data_name=slm.default_data_name,
